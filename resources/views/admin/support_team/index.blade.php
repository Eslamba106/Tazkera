        ...
        <tbody>
            @forelse ($team as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin.support_team.edit', $item->id) }}" {{-- <a href="{{ route('admin.categories.edit', $item->id) }}" --}}
                            class="btn btn-sm btn-outline-success">تعديل</a>
                    </td>
                    <td>
                        <a href="{{ route('admin.support_team.show', $item->id) }}" {{-- <a href="{{ route('admin.categories.edit', $item->id) }}" --}}
                            class="btn btn-sm btn-outline-info">عرض</a>
                    </td>
                    <td>
                        {{-- <form action="{{ route('admin.categories.delete', $item->id) }}" method="post"> --}}
                        <form action="{{ route('admin.support_team.delete', $item->id) }}" method="post">
                            @csrf
